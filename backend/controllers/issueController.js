...
//             { 'location.address': { $regex: req.query.state, $options: 'i' } }
//         );
//     }
...
const getIssues = asyncHandler(async (req, res) => {
    ...
    if (req.user) {
        ...
        const upvotedIds = new Set(
            userUpvotes.map(u => u.issue.toString())
        );
...
const updateIssue = asyncHandler(async (req, res) => {
    ...
    if (req.user.role === 'citizen') {
        res.status(403);
        throw new Error('Not authorized to update issues. Please contact an admin.');
    }
